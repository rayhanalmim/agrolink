...
function get_all_sub_filters(pid) {
  ...
  control.html("pdstatus", "Product added successfully");
}
...
function deleteadmin(id) {
  $.ajax({
    url: "assets/backend/admin/delete_admin.php",
    type: "post",
    data: "id=" + id,
    success: function (html) {
...
function add_neew_admin() {
  let username, pass, priv;
    ...
    $.ajax({
      ...
      success: function (html) {
        if (html == 1) {
          ...
          control.html("pdstatus", "This Username is already registered");
        }
      },
    });
  }
...
function filteradmin() {
  let inp = control.getInput("filteradmin");
  if (inp == "Super Admin") {
    inp = 1;
  } else if (inp == "Admin") {
    inp = 2;
  ...
  $.ajax({
    url: "assets/backend/admin/getspecified.php",
    type: "post",
    data: "id=" + inp,
    success: function (html) {
      control.html("sellersecroww", html);
